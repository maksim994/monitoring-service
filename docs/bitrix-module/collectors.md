...
Поддерживаемые имена архивов (как в мастере Bitrix): `.tar`, `.tar.gz`, `.tar.bz2`, `.enc`, `.enc.gz`, многотомные `.N`. Каталог `bitrix/backup/clouds/` не сканируется. Поиск — до 2 уровней вложенности (например `backup/archive/`).
...
Собирает только агенты с **ACTIVE=Y** (отключённые в админке Bitrix не учитываются).

- количество активных agents;
- количество просроченных agents;
- максимальный `NEXT_EXEC` lag;
- топ зависших agents (id, module, function) без чувствительных аргументов;
- тег `collector=agents_v3` в метриках;
- grace: просрочка учитывается только если `now - NEXT_EXEC` больше `max(120, 2 × AGENT_INTERVAL)` секунд;
- agents модуля `vendor.monitoring` (CollectorAgent) **не попадают** в `overdue_count` / алерт (отображаются отдельно в диагностике).

В настройках модуля — блок «Диагностика agents»: просроченные **активные** (алерт), **собственный** CollectorAgent (без алерта), отключённые с устаревшим NEXT_EXEC.
