...
export const CHECK_META: Record<string, CheckMeta> = {
  ...
  agents_lag: {
    ...
    description:
      'Фоновые задачи Bitrix (ACTIVE=Y): сверяется NEXT_EXEC с текущим временем. Отключённые в админке (ACTIVE=N) не учитываются.',
    whyItMatters:
      'Просроченные активные агенты — не уходят письма, не обновляется поиск. Часто причина — cron не настроен. Если агент отключён, а алерт остаётся — обновите модуль на сайте и отправьте метрику agents в облако.',
